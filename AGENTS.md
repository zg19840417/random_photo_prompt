...
**参考图拆分规则**：
- 用户给参考图要求拆分进提示词体系时，只拆镜头、姿势表情、场景光线、衣着或互动关系。
- 容貌身材维度必须始终使用用户预设的固定文本，不能根据参考图中的发色、发型、肤色、体型、人数或五官变化改写。
- 如果参考图是双人或多人画面，新增人物只能写在姿势/互动关系中，主角容貌身材仍保持固定预设。

**画面母题驱动方法论**：
- 提示词优化的首要目标不是"维度随机"，而是"最终画面差异"。每次生成必须先确定一个画面母题，再让镜头、衣着、姿势、场景、光线、色彩和调色服从该母题。
- 画面母题至少包含：时代/国家或地域、空间类型、时间段、主色与辅色、主要光源、服装风格、姿势家族、必须出现的道具或环境元素、禁止混入的元素。
- 维度池抽取时，主题一致性优先于单维度多样性。古代/民国/自然/夜店/沙丘/森林/船舫/酒廊等母题一旦确定，其他维度只能选择同主题或兼容主题的内容。
- 每个母题必须有互斥约束：古装母题禁止现代泳装、夜店灯带、玻璃小桌、现代酒廊；自然母题禁止镜面暗房、霓虹灯带和棚拍道具；夜店/酒廊母题禁止古代船舫、香炉、纱帐寝殿；沙丘/森林/湖畔母题禁止室内窗帘、地砖和吧台。
- 连续生成时需要控制差异度，不能连续抽到相同空间类型、相同时间段、相同主色、相同姿势家族和相同衣着轮廓。文本上不同但最终都会生成紫蓝镜面私房棚拍，应判定为失败。
- 自检必须做"结果反推审查"：读完整提示词后先判断模型最可能画成什么。如果不同母题最终都会被冷紫、淡粉、低雾、镜面反光、私房调色压成同一种画面，需要回到母题和互斥规则，而不是只改单条词。
- 二/三/四档的诱惑感应由母题内的光线、材质、姿势和表情共同完成；不能把冷紫、淡粉、幽蓝、低雾、镜面地面当作所有母题的固定公式。

**二/三/四档诱惑氛围规则**：
- 二档、三档和四档共用同一套独立的环境光与环境迷雾池，且只使用这套池子；不要再混入旧二档场景光线池。
- 二/三/四档可以使用冷紫色、淡粉色、幽蓝色、蓝紫霓虹、粉紫柔光、低位粉光、幽蓝暗部等迷离色彩，但这些颜色必须服务当前母题，不能覆盖所有场景。
- 二/三/四档场景中可以加入低雾、水汽、薄雾、纱帘雾感、镜面反射、湿润地面反光和丁达尔光束，但这些元素必须与母题兼容，不能让森林、沙丘、古代船舫、京都町屋等不同母题都变成同一类紫蓝镜面房。
- 诱惑氛围不等于只能暗光：清晨用冷白晨光、薄纱窗影和浅蓝灰阴影；正午用玻璃温室、泳池反光和强天光柔化；下午用暖白斜光、花园露台和水面反光；黄昏用金橙侧逆光、粉橙天空和边缘光；深夜才使用冷紫、淡粉、霓虹、床头灯、镜面暗房、浴室湿光和暗色地面反光。
- 二档、三档和四档即使是白天场景，也不能写成全场景明亮、背景完整展开、空间清楚交代。白天应使用"镜头压低曝光/背景欠曝虚化/局部阳光切到人物/窗光只落在脸、锁骨、腰线或腿部/环境只保留少量色块和反光"等描述，让画面突出人物而不是展示整个房间或整片风景。
- 避免在二/三/四档场景光线里使用"全屋通亮、明亮通透、强烈阳光照亮整个空间、完整背景层次、晴空完整展开、全场景高饱和"等会让模型把环境画得过于清楚的表述。
...
- 三档沿用二档姿势逻辑，但不组合衣着维度；二/三/四档的诱惑感主要由冷紫/淡粉/幽蓝环境光、迷雾、丁达尔光束、环境反光、手足前景、低机位和表情完成。
...
- 4档(nsfw)的quality尾巴与3档保持一致；四档只有姿势维度使用专用NSFW池，场景光线与2/3档共用专用迷离环境光/迷雾池，其余维度并入3档/2档逻辑
...
- 除非用户明确要求修改四档外挂姿势，否则不要改 `data/nsfw_pose_expression_options.json`；普通 1/2/3 档姿势需求必须落在项目内置姿势池。
- `data/nsfw_pose_expression_options.json` 内禁止出现眉心、眉头、眉间、眉眼、眉峰、眉毛、额心、额间、印堂等眉部/额心焦点描述；需要类似表情时改写为低头后抬眸、眼神微微上挑、眼尾看向镜头、眼角高光等可画表达。
