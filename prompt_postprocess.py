...
_FLAT_EXPRESSION_REPLACEMENTS = (
    ("嘴角放松没有明显笑容", "单侧嘴角轻轻上提，眼尾带一点俏皮嘲弄"),
    ("嘴角没有明显笑容", "单侧嘴角轻轻上提，眼尾带一点俏皮嘲弄"),
    ("嘴角没有笑容", "单侧嘴角轻轻上提，眼尾带一点俏皮嘲弄"),
    ("没有夸张笑容", "嘴角轻轻上扬"),
    ("眼神平静柔和", "眼尾轻弯，眼神柔和"),
    ("眼神平静直接", "眼尾微挑，眼神带玩味"),
    ("眼神安静专注", "眼神明亮专注"),
...
_EXPRESSION_BOOST_BY_SCALE = {
    ...
    "bold": ("单侧嘴角轻轻上提，眼尾带俏皮嘲弄", "薄唇轻抿成挑逗坏笑，狐狸眼斜看镜头", "嘴角像忍住笑一样上扬，眼神带玩味挑衅"),
    "bold_no_outfit": ("单侧嘴角轻轻上提，眼尾带俏皮嘲弄", "薄唇轻抿成挑逗坏笑，狐狸眼斜看镜头", "嘴角像忍住笑一样上扬，眼神带玩味挑衅"),
...
_BROKEN_PHRASE_REPLACEMENTS = (
    ...
    ("贴近镜头的半身肖像，头顶、脸部、肩线、锁骨和完整胸部可见", "贴近镜头的半身肖像"),
    ("贴近镜头的半身肖像，横向半身构图", "贴近镜头的半身肖像"),
    ("横向半身构图，。", "横向半身构图。"),
    ("肩膀及以上入镜。", "肩膀及以上入镜。"),
    ...
    ("左手的手指从颈侧滑过锁骨停在胸前的衣领边缘，带着勾引意味，双乳挺立，右手自然垂在身侧表情微笑", "左手从颈侧滑到锁骨下方，指尖停在衣领边缘，右手自然垂在身侧，低头，抬眼看向镜头，嘴角带浅淡微笑"),
    ...
    ("半身大幅侧弯", "半身侧弯，腰背形成S形曲线"),
    ("大腿以半身大幅侧弯", "大腿以上身体侧弯，腰背形成S形曲线"),
    ...
    ("肩颈和颈侧保持紧张", "肩颈向镜头轻轻靠近"),
    ("人物大腿以上竖向跪坐", "人物跪坐，身体略微侧转"),
    ("大腿以上竖向跪坐", "人物跪坐，身体略微侧转"),
...
# 质量行概念族去重：base(焦段/胶片) + grade(调色) + tail(肤质/高光安全句) 三层叠加时，
...
def _clean_instantiated_prompt_artifacts(text: str) -> str:
    ...
    replacements = (
        ...
        ("方形头部强透视近景", "头部近景"),
        ("竖向半身写真构图，竖向半身彩色棚拍近景", "竖向半身写真构图"),
        ("竖向半身写真构图，竖向半身", "竖向半身写真构图"),
        ("竖向半身彩色棚拍近景", "竖向半身写真构图"),
        ("横向竖向全身写真构图", "横向全身写真构图"),
        ("横向竖向", "横向"),
        ("横向平视腰部及以脸部优先清晰", "横向大腿以上入镜，脸部优先清晰"),
        ("宽画幅腰部及以人物斜向靠近画面一侧", "宽画幅半身构图，人物大腿以上斜向靠近画面一侧"),
        ...
        ("身体微微后仰，胸前衣料线条贴合身体，横向构图半身微侧", "身体微微后仰，胸前衣料线条贴合身体，半身微侧"),
    ...
    cleaned = re.sub(r"(竖向半身写真构图)，竖向半身[^，。]*", r"\1", cleaned)
    ...
    cleaned = re.sub(r"(全身像|半身像|头部特写)，", "", cleaned)
    cleaned = re.sub(r"，{2,}", "，", cleaned)
    cleaned = re.sub(r"。{2,}", "。", cleaned)
    return cleaned.strip("，。 \n\t")
...
def _move_head_half_hands_below_face(text: str, shot: str = "") -> str:
    ...
    for source, replacement in _HEAD_HALF_HAND_LOW_PLACEMENT_REPLACEMENTS:
        cleaned = cleaned.replace(source, replacement)
    low_hand = "一只手停在肩线下方" if shot == "head_shot" else "一只手停在胸前衣料边缘"
    low_edge = "肩线落在画面下缘" if shot == "head_shot" else "肩线和胸前衣料边缘"
    cleaned = re.sub(
...
def _remove_choice_language(text: str) -> str:
    cleaned = str(text or "")
    cleaned = cleaned.replace("或者", "或").replace("或是", "或")
    replacements = (
        ("支撑面边缘或胸前衣料边缘", "胸前衣料边缘"),
        ("边沿或胸前衣料边缘", "胸前衣料边缘"),
        ("脸侧或颈侧", "脸侧"),
        ...
        ("卷轴或铜镜", "铜镜"),
        ("卷轴或杯盏", "杯盏"),
        ("屏风、木窗、灯影或地面边缘", "屏风、木窗和灯影"),
        ("铜镜、烛火、雕花窗或薄纱帘", "铜镜、烛火和雕花窗"),
        ("屏风、花窗或竹影", "屏风和花窗"),
        ("屏风、低案、竹帘或雕花窗", "屏风、低案和竹帘"),
        ("宫灯、铜镜或木窗", "宫灯和铜镜"),
        ("屏风、低案、竹帘、雕花窗或木地面", "屏风、低案、竹帘和木地面"),
        ("画舫、宫苑、书房或竹林", "画舫、宫苑和书房"),
        ("自然光或柔和室内光", "自然光"),
        ("光线或纹理作用", "反光和纹理作用"),
        ("反光或纹理作用", "反光和纹理作用"),
    )
    ...
    for clause in clauses:
        clause = re.sub(r"([^，。；、\n]{1,18})或([^，。；、\n]{1,18})", r"\1", clause)
        clause = clause.replace("、或", "、").replace("或、", "、")
        fixed.append(clause.strip("，。；、 \n\t"))
    return "，".join(part for part in fixed if part).strip("，。；、 \n\t")


def _naturalize_clothing_edge_language(text: str) -> str:
    cleaned = str(text or "")
    replacements = (
        ("胸前衣料边缘", "胸前上衣"),
        ("肩头衣料边缘", "肩头上衣"),
...
def _dedupe_pose_sentence_language(text: str) -> str:
    cleaned = str(text or "")
    replacements = (
        ("头部回望镜头，头部从肩线旁回望镜头", "头部从肩线旁回望镜头"),
        ("头部回望镜头，头部回望镜头", "头部回望镜头"),
        ("回望镜头，头部从肩线旁回望镜头", "头部从肩线旁回望镜头"),
        ("头部俯视镜头，头部微低，抬眼看镜头", "头部微低，抬眼看镜头"),
        ("头部俯视镜头，低头，抬眼看镜头", "低头，抬眼看镜头"),
        ("俯视镜头，头部微低，抬眼看镜头", "头部微低，抬眼看镜头"),
        ("镜头，镜头", "镜头"),
        ("画面，画面", "画面"),
    )
    for source, replacement in replacements:
...
def _fix_unanchored_detail_language(text: str) -> str:
    cleaned = str(text or "")
    cleaned = cleaned.replace("黑色指甲清楚", "黑色指甲在胸前位置清楚")
    cleaned = cleaned.replace("黑亮指甲和指节高光清楚", "胸前的黑亮指甲带着小面积高光")
    cleaned = cleaned.replace("细长手指微弯，黑色指甲在胸前位置清楚", "细长手指在胸前微弯，黑色指甲可见")
    cleaned = cleaned.replace("细长手指微弯，黑色指甲清楚", "细长手指在胸前微弯，黑色指甲可见")
    cleaned = cleaned.replace("形成背景层次", "放在背景里")
    return cleaned
...
def clean_prompt_text(text: str) -> str:
    ...
    cleaned = _clean_instantiated_prompt_artifacts(cleaned)
    cleaned = _remove_choice_language(cleaned)
    cleaned = _naturalize_clothing_edge_language(cleaned)
    cleaned = _dedupe_pose_sentence_language(cleaned)
    cleaned = _fix_unanchored_detail_language(cleaned)
    ...
    cleaned = cleaned.replace("她用低头上目线盯住镜头", "低头，抬眼看向镜头")
    cleaned = cleaned.replace("低头上目线", "低头，抬眼")
    ...
    cleaned = cleaned.replace("人物大腿以上竖向坐立", "人物坐在画面中央，身体微侧转")
    cleaned = cleaned.replace("大腿以上竖向坐立", "人物坐在画面中央，身体微侧转")
    ...
    cleaned = cleaned.replace("胸部和肩颈在反弓姿态中显得更加突出头部微微后仰表情，带着挑衅的意味看向镜头颈线拉长", "半身微微后仰，胸前衣料贴合身体，肩颈向后拉开，头部微微后仰，颈线拉长，眼神带挑衅感看向镜头")
    ...
    cleaned = cleaned.replace("眼神直视镜头地注视镜头", "眼神直视镜头")
    cleaned = cleaned.replace("眼神直视镜头地", "眼神直视镜头")
    cleaned = cleaned.replace("眼神直视镜头注视镜头", "眼神直视镜头")
    cleaned = cleaned.replace("注视镜头", "直视镜头")
    cleaned = cleaned.replace("半身微微后仰，双手向后撑在身下，半身微微后仰", "半身微微后仰，双手向后撑在身下")
    ...
    cleaned = cleaned.replace("腰部及以肩颈、锁骨、胸部、细腰和双手同时可见", "大腿以上入镜")
    cleaned = cleaned.replace("腰部以半身，", "")
    cleaned = cleaned.replace("腰部以半身", "")
    ...
    cleaned = cleaned.replace(
        "半身侧躺姿态横向展开左手的手指从锁骨慢慢滑到胸前",
        "半身侧躺并横向展开，左手指尖从锁骨滑到胸前"
    )
    cleaned = cleaned.replace(
    ...
    cleaned = cleaned.replace("肩线和肩线下方", "肩线下方")
    cleaned = cleaned.replace("肩线和肩线下方", "肩线下方")
    cleaned = cleaned.replace("鼻梁、肩线和肩线下方", "鼻梁和肩线下方")
    cleaned = cleaned.replace("左手停在肩线下方的上衣", "左手停在肩线下方")
    cleaned = cleaned.replace("右手停在肩线下方的上衣", "右手停在肩线下方")
    cleaned = cleaned.replace("角色脚底及以上腰线", "角色脚底及以上全身镜头，腰线")
    cleaned = cleaned.replace("头部从树影下头部从肩线旁", "头部从树影下回望镜头")
    cleaned = cleaned.replace("角色脚底及以上机位", "角色脚底及以上全身镜头，机位")
    cleaned = cleaned.replace("双手左手", "左手")
    cleaned = cleaned.replace("湿润在亮处", "湿润皮肤在亮处")
    cleaned = cleaned.replace("贴合肩颈和胸前上衣", "贴合肩颈和胸前线条")
    cleaned = cleaned.replace("肩线侧转头部", "肩线侧转，头部")
    cleaned = cleaned.replace("前臂压住身前的台面", "肩膀向镜头前倾")
    cleaned = cleaned.replace("手掌撑住低台台面", "手掌撑在身侧")
    cleaned = cleaned.replace("低台台面", "身侧位置")
    cleaned = cleaned.replace("身前的台面", "身前位置")
    cleaned = cleaned.replace("手指停在胸前上衣", "左手横在胸前")
    cleaned = cleaned.replace("右手停在胸前上衣", "右手横在胸前")
    cleaned = cleaned.replace("指尖停在肩线下方", "一只手放在肩线下方")
    cleaned = cleaned.replace("手指停在肩线下方", "一只手放在肩线下方")
    cleaned = cleaned.replace("胸前的黑亮指甲带着小面积高光", "黑色指甲只作为手部小面积亮点")
    cleaned = cleaned.replace("把腿线拉出不对称的张力", "前后两条腿错开摆放")
    cleaned = cleaned.replace("眼尾上挑，眼尾带戏谑", "眼尾上挑，眼神带戏谑")
    cleaned = cleaned.replace("形成层次", "一前一后排开")
    cleaned = cleaned.replace("画面下缘只带到宋制近景造型", "画面下缘只露出宋制交领")
    cleaned = cleaned.replace("画面下缘只带到", "画面下缘只露出")
    cleaned = cleaned.replace("，裙摆下方保持裸足", "")
    cleaned = cleaned.replace("裙摆下方保持裸足，", "")
    cleaned = cleaned.replace("角色脚底及以上她", "角色脚底及以上全身镜头，她")
    cleaned = cleaned.replace("女孩她", "女孩")
    cleaned = cleaned.replace("女人她", "女人")
    cleaned = cleaned.replace("她她", "她")
    cleaned = cleaned.replace("两名女性站在灰色墙面前身体正面贴近", "两名女性身体正面贴近")
    ...
    cleaned = cleaned.replace("亮色墙面反光轻轻落在衣料，前景有亮色衣料", "亮色墙面反光轻轻落在肩颈和上身")
    cleaned = cleaned.replace("前景有亮色衣料", "前景有柔和亮色边缘")
    ...
    cleaned = re.sub(r"(头部|半身|半身|大腿以上|全身)[^，。]{0,8}(清楚|完整呈现)", r"\1构图", cleaned)
    cleaned = cleaned.replace("半身肖像构图，竖向半身花园水光构图，大腿以上入镜", "竖向半身构图，大腿以上入镜")
...
def _strip_no_outfit_clothing_terms(parts: dict[str, str], scale: str = "") -> dict[str, str]:
    cleaned = dict(parts)
    if scale not in {"bold_no_outfit", "nsfw"}:
        return cleaned
    for name in ("director_plan", "pose_expression", "scene_light", "quality"):
        text = str(cleaned.get(name) or "")
        text = text.replace("胸前衣料边缘", "胸前")
        text = text.replace("肩头衣料边缘", "肩头")
        text = text.replace("肩线下方衣料边缘", "肩线下方")
        text = text.replace("衣料边缘", "皮肤边缘")
        text = text.replace("衣料纹理", "皮肤纹理")
        text = text.replace("胸前上衣", "胸前")
        text = text.replace("肩头上衣", "肩头")
        text = text.replace("肩线下方的上衣", "肩线下方")
        text = text.replace("上衣纹理", "皮肤和发丝")
        text = text.replace("上衣", "身体")
        text = text.replace("衣料", "皮肤")
        text = text.replace("衣摆", "腿侧")
        text = text.replace("裙摆", "腿侧")
        cleaned[name] = text
    return cleaned
...
def clean_global_prompt_text(parts: dict[str, str], shot: str = "", scale: str = "") -> dict[str, str]:
    ...
    if not any(marker in scene_for_pose for marker in ("吧台", "酒吧", "酒廊", "吧椅", "黄铜麦克风")):
        ...
        pose = pose.replace("吧椅前缘", "高凳前缘")
        pose = pose.replace("吧椅横档", "高凳横档")
        pose = pose.replace("黑色吧台边缘", "身前支撑面边缘")
        pose = pose.replace("黑色吧台", "身前支撑面")
        pose = pose.replace("吧台边缘", "身前支撑面边缘")
        pose = pose.replace("吧台反光边", "身前支撑面边缘")
        pose = pose.replace("威士忌杯", "细口玻璃杯")
        cleaned["pose_expression"] = pose
    if not any(marker in scene_for_pose for marker in ("泳池", "池边", "池水", "湖蓝水面", "水面")):
        pose = str(cleaned.get("pose_expression") or "")
        pose = pose.replace("泳池边缘", "低台边缘")
        pose = pose.replace("池边瓷砖", "低台台面")
        pose = pose.replace("池边", "低台边")
        cleaned["pose_expression"] = pose
    if not any(marker in scene_for_pose for marker in ("浴室", "镜", "水汽", "瓷砖")):
        pose = str(cleaned.get("pose_expression") or "")
        pose = pose.replace("头部贴近雾面镜边缘，指尖停在镜面水汽旁，眼神越过镜中暗光看向镜头", "头部贴近窗边浅色墙面，指尖停在肩线下方，眼神从侧光里看向镜头")
        pose = pose.replace("雾面镜边缘", "浅色墙面边缘")
        pose = pose.replace("镜面水汽旁", "肩线下方")
        pose = pose.replace("镜中暗光", "侧光")
        cleaned["pose_expression"] = pose
    if not any(marker in scene_for_pose for marker in ("木栈道", "湿木板", "栈道", "湖边", "森林", "木板")):
        pose = str(cleaned.get("pose_expression") or "")
        pose = pose.replace("湿木栈道边缘", "低台边缘")
        pose = pose.replace("木板上", "低台边")
...
def _dedupe_full_body_camera(text: str) -> str:
    ...
    if not clauses:
        return str(text or "")
    # 防御：若分句里已有更具体的全身描述，去掉泛化残片「竖向全身构图」
    if any("全身" in c and c != "竖向全身构图" for c in clauses):
        clauses = [c for c in clauses if c != "竖向全身构图"]
...
def _fix_scene_ground_anchor(text: str) -> str:
    ...
    if "人物轮廓边缘有轻柔高光" in cleaned:
        cleaned = cleaned.replace("，自然反光落在人物边缘", "")
        cleaned = cleaned.replace("自然反光落在人物边缘，", "")
        cleaned = cleaned.replace("照亮脸、唇、下颌、颈部和双肩边缘，人物轮廓边缘有轻柔高光", "照亮脸、唇、下颌、颈部和双肩，人物轮廓有轻柔高光")
    if "托亮边缘" in cleaned and "人物轮廓边缘" in cleaned:
        cleaned = cleaned.replace("托亮边缘，人物轮廓边缘", "托亮人物轮廓，人物边缘")
    return cleaned
...
def _clean_pose_visual_language(text: str) -> str:
    ...
    pose = re.sub(r"^(全身像|半身像|半身像|大腿以半身|大腿以半身|肩部以上入镜|半身近景|头顶完整)，?", "", pose)
