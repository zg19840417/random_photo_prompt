...
class EraPromptGenerationTest(unittest.TestCase):
    ...
    def test_krea2_portrait_workflow_adds_orientation_guard(self):
        ...
        self.assertIn("strict upright vertical portrait", source)
    ...
    def test_external_krea2_script_preserves_exact_prompt(self):
        server_source = (ROOT / "__init__.py").read_text(encoding="utf-8")
        client_source = (ROOT / "tools" / "generate_remote_image.py").read_text(encoding="utf-8")

        self.assertIn('"exact_prompt": True', client_source)
        self.assertNotIn('"shot": "full_body"', client_source)
        self.assertIn('exact_prompt = manual_only and data.get("exact_prompt") is True', server_source)
        self.assertIn("apply_krea2_orientation_guard=not exact_prompt", server_source)

    def test_mobile_krea2_preserves_manual_and_pregenerated_prompts(self):
        server_source = (ROOT / "__init__.py").read_text(encoding="utf-8")
        mobile_source = (ROOT / "web" / "mobile.html").read_text(encoding="utf-8")

        self.assertIn('custom_prompt_source in {"manual", "pregenerated"}', server_source)
        self.assertIn("and not exact_prompt and not custom_prompt", server_source)
        self.assertIn('["manual", "pregenerated"].includes(state.customPromptSource)', mobile_source)

    def test_mobile_krea2_cc_keeps_fixed_lora_chain(self):
        import json

        server_source = (ROOT / "__init__.py").read_text(encoding="utf-8")
        mobile_source = (ROOT / "web" / "mobile.html").read_text(encoding="utf-8")
        workflow = json.loads((ROOT / "mobile_workflow_api_krea2_cc.json").read_text(encoding="utf-8"))

        self.assertIn('"krea2_cc": {"label": "单采-Krea2+CC"', server_source)
        self.assertIn('if workflow_key == "krea2_cc":', server_source)
        self.assertIn('return _append_mobile_lora_nodes(workflow, terminal_node_id, terminal_node, loras)', server_source)
        self.assertIn('<option value="krea2_cc">单采-Krea2+CC</option>', mobile_source)
        self.assertEqual(
            [workflow[str(node_id)]["inputs"]["strength_model"] for node_id in (20, 21, 22, 23)],
            [0.5, 1.0, 0.7, 0.3],
        )
        self.assertEqual(workflow["3"]["inputs"]["model"], ["23", 0])

    def test_prompt_has_visual_director_plan(self):
        item = generate_prompt_items(
            1,
            {"scale": "bold", "shot": "full_body", "aspect": "portrait", "era": "modern"},
            seed_text="visual-director-plan",
        )[0]
        parts = item["dimension_parts"]
        prompt = item["positive_prompt"]

        self.assertEqual(parts.get("director_plan_locked"), "1")
        self.assertIn("director_plan", parts)
        self.assertIn("画面重心", prompt)
        self.assertIn("视觉路径", prompt)
        self.assertIn("光线落点", prompt)
        self.assertIn("材质统一", prompt)
