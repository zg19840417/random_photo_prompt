...
This document does not define image-to-video motion option formats. Use:
...
K2 是 `/Users/zouge/Downloads/K2人像提示词引擎（SFW版）.md` 的独立还原，不读取或继承项目规则的档位、年代、镜头、固定人物、六维度池或负面提示词。手机端选中 K2 后隐藏档位、年代和镜头；后端忽略这些请求字段，并按 K2 本次随机出的镜头范围、视角和构图自动选择分辨率。优先级为：鸟瞰使用 `1536×1536`；其余广角、超广角和鱼眼使用 `1920×1280`；非广角的中心对称构图使用 `1536×1536`；中景使用 `1216×1664`；近景使用 `1088×1920`。
...
项目默认规则固定人物为“22岁瓷白冷白皮K-pop韩国美女”。固定容貌描述只包含五官与体形：肤质细腻如瓷、细长瓜子脸、非常尖的下巴、高挺精致鼻梁与鼻尖尖翘；黑色直发散落在脸部两侧，魅惑狐狸眼型；性感锁骨、骨架偏瘦但胸部和臀部丰满、小蛮腰、腿细且长。妆容不进入 character 维度，由独立 makeup 维度生成（眼线/眼影/睫毛/美瞳/唇彩/指甲等）。

人物手部进入镜头时，固定指甲描述必须为“又细又长的黑色手指甲点缀反光银粉”。身体描述仍必须按镜头可见范围裁剪，不可向较小镜头补写不可见部位。
...
The mobile page uses the in-memory current-session job list for queue progress and prompt association. Completed remote mobile images should be atomically written into the local Mac ComfyUI `output` directory, and the mobile gallery should recursively read local images below that directory by file modification time, newest first. The remote result WebSocket must use an explicit message-size limit of at least 64 MiB; the aiohttp default is too small for high-resolution PNG results.
