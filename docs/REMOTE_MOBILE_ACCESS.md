...
文生视频和图生视频页面不提供档位选择，不保存或提交视频档位，也不继承图片档位；自定义视频提示词仍原样提交。

视频参考图的选择、重新选择、上传和清除均不改动当前动作提示词，也不自动请求生成提示词。文生视频和图生视频页不提供“生成提示词”按钮，动作描述由用户手动输入；提交时空输入的处理遵循既有视频提交规则。

图生视频选图弹窗提供“默认 / 收藏”页签，分别读取本机普通图库和收藏备份；任一页签为空仍可切换另一页签。点击图片完成单选，收藏图片即使原图已删除也能作为首帧，使用时仅复制到 Mac 本机视频 input 目录。

视频瀑布流详情展示“视频提示词”，只使用该视频任务实际提交的 `generation_prompt`，不读取参考图片提示词。回传落盘时按视频子目录和文件名写入 Mac 本地提示词索引，图库通过 `video_prompt` 字段提供；现存任务可恢复记录，缺失时明确显示“未记录视频提示词”。复制按钮仅复制提示词正文。

视频详情提供“循环播放：开/关”按钮，默认关闭，使用浏览器原生视频循环播放。在当前页面内切换视频、关闭再打开详情均保留开关状态；刷新页面恢复关闭。图片详情不显示此按钮。原有延迟 1 秒静音自动播放行为保持不变。
...
桌面“重启远端 ComfyUI.command”只负责双端服务编排，不执行本机推理，也不清理非 ComfyUI 进程。执行顺序固定为：①同步本机唯一源码并重启 Windows 4090；②重启 Mac 本机 8188 编排/回传服务；③根据到远端的当前路由计算手机入口；④读取远端可用 RAM 与各 GPU 显存。每阶段和总耗时写入 `/tmp/random_photo_prompt_restart_latest.log`；资源报告中的 RAM“可用”来自 Windows `FreePhysicalMemory`，显存“可用/已用/总计/利用率”来自 `nvidia-smi`。脚本失败会记录失败阶段和累计耗时，不应据此推断远端正在执行推理。

远端 Windows ComfyUI 启动参数包含 `--fast-disk`。视频模型及 MiniMax 文本编码器位于 E 盘 SSD，动态加载优先使用磁盘后端，避免 64GB 级系统内存因 pinned memory 与模型暂存叠加而触发 `HostBuffer.read_file_slice failed`；不得改回默认内存优先加载。
